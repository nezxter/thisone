...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Effects Controls
    // These elements no longer exist in the UI, so we'll create dummy values
    let delayControlValue = 0;
    let reverbControlValue = 0;
    ...
    function updateEffects() {
        // UI elements no longer exist, using the stored values instead
        delayGain.gain.value = delayControlValue * 0.7;
        reverbGain.gain.value = reverbControlValue;
        
        console.log("Effects updated - Delay:", delayGain.gain.value, "Reverb:", reverbGain.gain.value);
    }
    
    // Apply a sound mode
    function applySoundMode(modeName) {
        if (!soundModes[modeName]) return;
        
        const settings = soundModes[modeName];
        ...
        // Update the UI
        delayControlValue = settings.delay;
        reverbControlValue = settings.reverb;
        ...
        console.log("Sound mode applied:", modeName, "Waveform:", currentWaveform);
    }
    ...
    function updateDebug() {
        ...
        debugEl.textContent = `AudioContext: ${audioCtx.state} | Waveform: ${currentWaveform} | Delay: ${delayControlValue.toFixed(2)} | Reverb: ${reverbControlValue.toFixed(2)}`;
    }
